...
Often the clusters produced by dendrograms have different labels than those produced by `stats::cutree` resulting in different cluster labels for `sequenchr::plot_dendrogram` and other `sequenchr::plot_*` functions. This is mitigated by using `sequenchr::cluster_labels` to cut the tree into k clusters. The resulting numbered labels match the dendrogram labels left-to-right.
